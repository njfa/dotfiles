...
共通操作は `<leader>v`（Space → v）配下にまとめている。ノーマルモードで使用し、
プレフィックスを入力して待つとwhich-keyで操作一覧を確認できる。

| キー | 操作 |
| --- | --- |
| `<leader>vb` | ブレークポイント切替 |
| `<leader>vB` | 条件付きブレークポイント |
| `<leader>vc` | 開始、構成選択、再開 |
| `<leader>vn` | step over：次の行へ進む |
| `<leader>vi` | step into：関数の中へ入る |
| `<leader>vo` | step out：現在の関数から抜ける |
| `<leader>vs` | セッション一覧 |
| `<leader>vq` | セッション終了 |
| `<leader>vu` | DAP UI切替 |
...
launch方式を使うときだけJavaバッファで `mdc` を押し、生成された構成を `<leader>vc` で選ぶ。
...
実行中のSpring Bootへattachする場合は、サービスをデバッグポート付きで起動して `<leader>vc` から
「実行中のJVMにアタッチ」を選ぶ。
