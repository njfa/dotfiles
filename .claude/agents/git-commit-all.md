...
1. `git status --porcelain`を実行し、変更されたファイルと未追跡ファイルを確認する
2. 修正されたファイル（Modified）に対し、`git diff --no-ext-diff <ファイル名>`を実行する
3. 未追跡ファイル（Untracked）に対し、ファイル内容を`cat`コマンドで表示する
4. 1-3の取得に失敗した場合、変更がないことを表示して処理を終了する
