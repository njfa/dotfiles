...
description: ステージングされた変更差分を取得し、それを機能単位で適切に分割したコミットを作成する。
...
- **mcp__serenaツールの積極的活用**: プロジェクト構造を理解するためにmcp__serenaのツール群を優先的に使用
  - `mcp__serena__list_dir`: ディレクトリ構造の把握と変更箇所の分類
  - `mcp__serena__get_symbols_overview`: コードシンボルの概要取得と変更の影響範囲把握
  - `mcp__serena__find_symbol`: 特定シンボルの詳細取得と関連変更の特定
  - `mcp__serena__read_memory`: プロジェクト固有のコミットルールや方針の参照

- **CLAUDE.md参照**: プロジェクト固有のルールや方針がCLAUDE.mdに記載されている場合は必ず参照し、そのガイドラインに従ってコミットメッセージを作成

## 必須実行手順

### 1. 変更差分の取得フェーズ

ステージングされているgitの差分を取得します。

1. `git diff --no-ext-diff --staged --name-only`を実行し、コミット対象のファイルを確認する。
2. 各ファイルに対し、`git diff --no-ext-diff --staged <ファイル名>`を実行する
3. 1もしくは2の取得に失敗した場合、変更のステージングが必要であることを表示して処理を終了する

このとき、`git diff --no-ext-diff`を実行してステージングされていない変更を参照することは禁止とする。
