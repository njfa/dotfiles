...
- **mcp__serenaツールの積極的活用**: プロジェクト構造を理解するためにmcp__serenaのツール群を優先的に使用
  - `mcp__serena__list_dir`: ディレクトリ構造の把握とコード品質の全体的な評価
  - `mcp__serena__get_symbols_overview`: コードシンボルの概要取得と命名規則の分析
  - `mcp__serena__find_symbol`: 特定シンボルの詳細取得と複雑度の確認
  - `mcp__serena__read_memory`: プロジェクト固有のコーディング規約や方針の参照

- **CLAUDE.md参照**: プロジェクト固有のルールや方針がCLAUDE.mdに記載されている場合は必ず参照し、そのガイドラインに従ってコード品質レビューを実施
